use std::{error::Error, fs::File, io::Write, path::Path, path::PathBuf};
...
impl Generator {
  ...
  pub fn generate(
    &self,
    icons: impl Iterator<Item = (IconFile, IconData)>,
    ...
    wln!(out, "");
    wln!(out, "///");
    wln!(out, "/// This code is generated by `druid-icon-generator`.");
    wln!(out, "///");
    wln!(out, "");
...
impl Generate for (IconFile, IconData) {
  fn generate(&self, out: &mut impl Write, ident: usize) -> Result<(), Box<dyn Error>> {
    ...
    wlni!(
      out,
      ident,
      "pub const {}: IconStaticData = IconStaticData {{",
      const_name
    );
    wlni!(
      out,
...
impl Generate for IconPath {
  fn generate(&self, out: &mut impl Write, ident: usize) -> Result<(), Box<dyn Error>> {
    ...
    wi!(out, ident, "  transform: ");
    self.transform.generate(out, ident)?;
    writeln!(out, ",")?;
    wi!(out, ident, "  fill: ");
    self.fill.generate(out, ident)?;
    writeln!(out, ",")?;
    wi!(out, ident, "  stroke: ");
    self.stroke.generate(out, ident)?;
    writeln!(out, ",")?;
...
impl Generate for Affine {
  fn generate(&self, out: &mut impl Write, _ident: usize) -> Result<(), Box<dyn Error>> {
    ...
    coeffs
      .iter()
      .take(1)
      .try_for_each(|coeff| write!(out, "{:?}", coeff))?;
    coeffs
      .iter()
      .skip(1)
      .try_for_each(|coeff| write!(out, ", {:?}", coeff))?;
...
impl Generate for IconPathStroke {
  fn generate(&self, out: &mut impl Write, _ident: usize) -> Result<(), Box<dyn Error>> {
    write!(
      out,
      "IconPathStroke {{ opacity: {:?}, width: {:?} }}",
      self.opacity, self.width
    )?;
    Ok(())
  }
}
