...
pub enum Reference {
  Source(#[data(same_fn = "PartialEq::eq")] SourceRef),
  Param(#[data(same_fn = "PartialEq::eq")] ParamRef),
}
...
impl Modulations {
  ...
  pub fn stop_config(&mut self, source_ref: SourceRef) {
    ...
    self.config_source = self.config_source.filter(|v| *v != source_ref);
  }
  ...
  fn groups<NR>(
    ...
    base_groups: &Vector<NR>,
  ) -> Vector<Group>
  where
    NR: NamedReference + Clone,
  {
    ...
    let mut groups = HashMap::<usize, Group>::from_iter(base_groups.iter().map(|nr| {
    ...
    for (index, internal_modulation) in self.modulations.iter().enumerate() {
      ...
      let modulation =
        internal_modulation.as_modulation(index, modulation_name, self.synth_client.clone());
  ...
  {
    let mut groups =
      HashMap::<usize, usize>::from_iter(base_groups.iter().map(|nr| (nr.reference().into(), 1)));
  ...
  pub fn add_modulation(&mut self, modulation: InternalModulation) {
    ...
    self
      .synth_client
      .send_modulation_update(
        modulation.source_ref,
        modulation.param_ref,
        modulation.amount as f32,
      )
      .unwrap();
  ...
  pub fn delete_modulation(&mut self, source_ref: SourceRef, param_ref: ParamRef) {
    self
      ...
      .iter()
      .for_each(|index| {
        self.modulations.remove(*index);
      });

    self
      .synth_client
      .send_modulation_delete(source_ref, param_ref)
      .unwrap();
  }
}
...
impl ListIter<Group> for Modulations {
  ...
  fn data_len(&self) -> usize {
    match self.view {
      View::GroupBySource => self.count(|m| Reference::Source(m.source_ref), &self.sources),
      View::GroupByParam => self.count(|m| Reference::Param(m.param_ref), &Vector::<()>::new()),
    }
  }
}
