...
use druid::{Widget, Env, PaintCtx, RenderContext};
use druid::widget::{Flex, WidgetExt, FillStrat, Label, Container, Painter};
...
impl HeaderView {
  ...
  fn paint_voices(ctx: &mut PaintCtx, num_voices: &usize, _env: &Env) {
    ...
    ctx.fill(Rect::new(margin, margin, margin + fill_width, size.height - margin), &GREY_83);
  }
}
